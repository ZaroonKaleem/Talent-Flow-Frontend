...
export class EmployeeJobFieldService {
private apiUrl = `${environment.apiUrl}services/app/JobField`;
   ...
   */
  createJobField(jobField: { id: number; name: string }): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
  ...
  deleteJobField(id: number): Observable<any> {
    ...
    updateEmployeeJob(jobField: {
        ...
        return this.http.put<any>(`${this.apiUrl}/Update`, jobField, {
            headers,
        });
  }
}
