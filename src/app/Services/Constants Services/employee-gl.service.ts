...
export class EmployeeGlService {
private apiUrl = `${environment.apiUrl}services/app/GLClass`;
  ...
  getAllEmployeeBanks(params?: any): Observable<any> {
    ...
    return this.http.get<any>(`${this.apiUrl}/GetAll`, {
      ...
      createGlClass(GlClassData: {
        ...
        return this.http.post<any>(`${this.apiUrl}/Create`, GlClassData, {
            headers,
        });
    }

    deleteGlClass(id: number): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
