...
private apiUrl = `${environment.apiUrl}services/app/ClientCustomer`;
   ...
   */
  createClientCustomer(clientCustomer: { id: number; name: string }): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    ...
    return this.http.post<any>(`${this.apiUrl}/Create`, clientCustomer, { headers });
  }

  deleteClientCustomer(id: number): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
     ...
     */
    updateClientCustomer(clientCustomer: {
        id: number;
        name: string;
    }): Observable<any> {
        const token = localStorage.getItem('accessToken');
        ...
        return this.http.put<any>(`${this.apiUrl}/Update`, clientCustomer, {
            headers,
        });
    }
}
