...
export class EmployeeExpenseUnitService {
private apiUrl = `${environment.apiUrl}services/app/ExpenseUnit`;
  ...
  getAllEmployeeStations(params?: any): Observable<any> {
   ...
   createExpenseUnit(expenseUnitData: {
        id: number;
        name: string;
    }): Observable<any> {
        const token = localStorage.getItem('accessToken');
        ...
        return this.http.post<any>(`${this.apiUrl}/Create`, expenseUnitData, {
            headers,
        });
    }

    deleteExpenseUnit(id: number): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    ...
    return this.http.delete<any>(`${this.apiUrl}/Delete`, {
     ...
     /**
       ...
       */
      updateExpenseUnit(expenseUnitData: {
          id: number;
          name: string;
      }): Observable<any> {
          const token = localStorage.getItem('accessToken');
          ...
          return this.http.put<any>(`${this.apiUrl}/Update`, expenseUnitData, {
              headers,
          });
    }
}
