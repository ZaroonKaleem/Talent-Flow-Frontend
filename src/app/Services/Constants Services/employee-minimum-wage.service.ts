...
export class EmployeeMinimumWageService {
private apiUrl = `${environment.apiUrl}services/app/MinimumWage`;
  ...
  getAllEmployeeStations(params?: any): Observable<any> {
    ...
    return this.http.get<any>(`${this.apiUrl}/GetAll`, {
      ...
      params: httpParams
       ...
       createMinimumWage(minimumWageData: {
        id: number;
        name: string;
        provinceId: number;
    }): Observable<any> {
        ...
        return this.http.post<any>(`${this.apiUrl}/Create`, minimumWageData, {
            headers,
        });
    }

    deleteMinimumWage(id: number): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
