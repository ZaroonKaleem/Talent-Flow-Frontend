...
export class EmployeePrefixService {
private apiUrl = `${environment.apiUrl}services/app/EmployeePrefix`;
  ...
  getAllEmployeeStations(params?: any): Observable<any> {
    ...
    createPrefix(employeePrefix: { id: number; name: string }): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    ...
    return this.http.post<any>(`${this.apiUrl}/Create`, employeePrefix, { headers });
  }

  deleteEmployeePrefix(id: number): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
