...
export class EmployeeExitTypeService {
private apiUrl = `${environment.apiUrl}services/app/ExitType`;
  ...
  createExitType(exitTypeData: {
    ...
    deleteExitType(id: number): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    ...
    updateExitType(exitTypeData: {
        ...
        return this.http.put<any>(`${this.apiUrl}/Update`, exitTypeData, {
            headers,
        });
  }
}