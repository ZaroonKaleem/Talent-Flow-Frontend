...
export class EmployeeDesignationService {
  private apiUrl = `${environment.apiUrl}services/app/Designation`;
   ...
   */
  createDesignation(designation: { id: number; name: string; code: string }): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    ...
    return this.http.post<any>(`${this.apiUrl}/Create`, designation, { headers });
  }

    deleteEmployeeDesignation(id: number): Observable<any> {
    const token = localStorage.getItem('accessToken');
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
